const getAdjacentCharacter = (char: string, num: number): string =>
  String.fromCharCode(char.charCodeAt(0) + num);

const availableMoves = (position: string) => {
  if (!position) return []; // If cell not selected, return empty array
